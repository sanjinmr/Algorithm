...
public class DirectInsertSort {
    ...
    public static void insertShort(int [] a,  int n) {
        ...
        for (i= 1; i < n; i++) {
            //第一步：搜索
            //在待排元素a[i]前面的a[0...i-1]有序区间从后往前搜索一个合适的位置 ：倒数第一个不大于a[i]的位置；
            //如果找到了那个位置是j，a[i]的插入位置就是后面的那个位置:j + 1；
            //如果从后往前搜索到最前也没有发现，则j的值此时变为-1了，则将待排元素插入：j + 1 = 0，即数组最前面
            for (j = i - 1; j >= 0; j --) {
                if (a[j] > a[i]) {
                    // 如果大于待排元素则继续往前搜索
                } else {
                    // 如果找到倒数第一个不大于a[i]的元素，则结束遍历有序区间，可以去搬移、插入数据了
                    break;
                }
            }

            //第二步：数据移动
            // 如j==i-1，说明倒数第一个不大于a[i]的位置就是a[i]前面一个位置的元素，
